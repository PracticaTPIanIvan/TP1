...
public class Game {
	...
	public boolean receiveExplosion(int x, int y) {
		return objectContainer.receiveExplosion(x, y);
	}
	...
	public void advanceFirstObj(int x, int y) {
		(objectContainer.getFirstObj(x, y)).advance();
	}
	...
	public void removeDead() {
		...
		for (int i = getPlayerPositionX(); i < getRoadLength(); i++) {
			for (int j = 0; j < getRoadWidth(); j++) {
				object = objectContainer.getNextObj(i, j, numObjChecked);
				while (object != null) {
					if (object.isAlive()) {
						aux.add(objectContainer.getNextObj(i, j, numObjChecked)); 
	...
	public String positionToString(int x, int y) { 
		...
		while (numObjInPos > numObjChecked) {
			icon += objectContainer.nextObjToString(x, y, numObjChecked);
			numObjChecked++;
		}
